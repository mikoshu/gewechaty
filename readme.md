...
更多功能尚在开发（以下为已实现功能，更新后api可能会修改，谨慎使用）
...
```
注意 登录成功后 appId uuid 和 token 将保存在当前项目根目录的ds.json文件中，如果项目迁移但是Gewechat没有退出登录的话需要将文件迁移过新项目根目录下。

### 2. Message 消息对象
基本支持wechaty的Message方法
...
注意：此处所有涉及到发送的文件url的都必须为`http://` 开头的地址，可将文件放在静态托管目录下，即可使用`http://ip:port/xxx/xxx.png`的方式传输
